    ...
    public static class GameObjectExtensions
    {
        ...
        public static void EmbedInSurface(this GameObject gameObject, Vector3 embedDirection, GameObject embedSurface)
        {
            ...
            if (worldOOBB.IsValid())
            {
                ...
                {
                    // For meshes (e.g. sphere) if helps to know if the point is lying underneath the mesh
                    // surface. Otherwise, the object will be projected on the other side of the mesh.
                    ray = new Ray(corner, -embedDirection);
                    if (embedSurface.RaycastTerrainOrMesh(ray, out rayHit)) continue;
                ...
                if (needToMove) gameObject.transform.position += embedDirection * Mathf.Sqrt(maxDistSq);
            }
        }
